...
class PlotInfo {
  ...
  PlotInfo({
    ...
    totalAmount = fare * 100;
  }

  // CLONE CONSTRUCTOR FOR IMMUTABILITY
  PlotInfo.clone(PlotInfo other)
      : id = other.id,
        area = other.area,
  ...
  double _parseArea(String area, AreaUnit unit) {
    ...
    String cleaned = area.replaceAll(RegExp(r'[a-zA-Z]'), '').replaceAll(RegExp(r'[^0-9.]'), '');
  ...
  // COLOR LOGIC (based on payment + approval status via bookingStatus)
  Color get color {
    switch (bookingStatus) {
      case BookingStatus.available:
        return Colors.green.shade300; // Available

      case BookingStatus.pending:
        if (paidPercentage < 50) return Colors.red.shade300; // <50%
        if (paidPercentage >= 50 && paidPercentage < 100) return Colors.yellow.shade300; // ≥50%
        if (paidPercentage >= 100) return Colors.grey.shade400; // Full paid (direct sellout)
        break;

      case BookingStatus.booked:
        return Colors.blue.shade300; // Approved <50%
      case BookingStatus.sellout:
        return Colors.grey.shade400; // Approved ≥50% or full paid
    }
  ...
  String get statusText {
    switch (bookingStatus) {
      ...
      case BookingStatus.pending:
        ...
        if (paidPercentage >= 100) return "Sold Out";
        break;
      case BookingStatus.booked:
        return "Booked";
      case BookingStatus.sellout:
  ...
  Map<String, dynamic> toJson() {
    ...
    final bookedAreaValue = bookedArea;
    final remaining = (totalAreaValue - bookedAreaValue).toString();
    final totalAmountValue = fare * 100;
    ...
    return {
      ...
      'Booking_Area': bookedAreaValue.toStringAsFixed(1),
      'Screenshot': screenshot,
      'Total_Amount': totalAmountValue.toInt(),
  ...
  factory PlotInfo.fromJson(Map<String, dynamic> json) {
    ...
    final fare = totalAmount > 0 ? totalAmount / 100 : 0.0;
...
class _BookingDialogState extends State<BookingDialog> {
  ...
  void initState() {
    ...
    _purchasePriceController = TextEditingController(text: widget.plot.purchasePrice.toStringAsFixed(0));
    _receivingController = TextEditingController(text: widget.plot.receivingAmount.toStringAsFixed(0));
    ...
    _controllers = [
      _clientController, _areaController, _bookedAreaController, _remainingAreaController,
      _fareController, _paidController, _projectController, _purchasePriceController,
      _receivingController, _pendingController, _paidThroughController, _bookedByController,
      _customerPhoneController, _dealerPhoneController, _bookingDateController, _plotTypeController,
    ];
  ...
  void _updateRemainingArea() {
    final totalText = _areaController.text;
    final bookedText = _bookedAreaController.text;

    final total = double.tryParse(totalText) ?? 0;
    final booked = double.tryParse(bookedText) ?? 0;
    ...
    for (int i = 1; i <= 3; i++) {
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          await Future.delayed(Duration(seconds: i * 2)); // retry delay
        }
      } catch (e) {
                    ...
                    children: [
                          ...
                          Expanded(
                            child: _buildTextField(
                              ...
                              validator: (value) {
                                ...
                                final bookedArea = double.tryParse(value) ?? 0;
                                final totalArea = double.tryParse(_areaController.text) ?? 0;
                                final unitFactor = _areaUnit == AreaUnit.sqFt ? 1/9.0 : 1.0;
                                final totalInYds = totalArea * unitFactor;
                                if (bookedArea > totalInYds) {
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _isLoading || !_isSubmitEnabled
                              ? null
                              : () async {
                            if (_formKey.currentState!.validate()) {
                              ...
                              localPlot.paidAmount = double.tryParse(_paidController.text) ?? 0;
                              localPlot.bookedArea = double.tryParse(_bookedAreaController.text) ?? 0;
                              ...
                              localPlot.purchasePrice = double.tryParse(_purchasePriceController.text) ?? 0;
                              localPlot.receivingAmount = double.tryParse(_receivingController.text) ?? 0;
                              localPlot.pendingAmount = totalAmountValue - localPlot.paidAmount;
                              localPlot.paidThrough = _paidThroughController.text;
                              ...
                              localPlot.bookingDate = DateTime.tryParse(_bookingDateController.text) ?? DateTime.now();
                              localPlot.plotType = _plotTypeController.text;

                              if (localPlot.bookedArea == 0 && localPlot.paidAmount == 0) {
                                localPlot.bookingStatus = BookingStatus.available;
                              } else if (localPlot.paidAmount >= localPlot.totalAmount) {
                                localPlot.bookingStatus = BookingStatus.sellout;
                              } else {
                                localPlot.bookingStatus = BookingStatus.pending;
                              }

                              try {
                                final response = await _submitBookingAPI(localPlot);
...
class _PlotLayoutScreenState extends State<PlotLayoutScreen> {
  ...
  void initState() {
    ...
    _initializePlots();
    _fetchPlotData();
    //RealTimeService.listen((updatedPlot) {
    //if (mounted && plots.containsKey(updatedPlot.id)) {
    // setState(() {
    // plots[updatedPlot.id] = updatedPlot;
    //  print('REAL-TIME UPDATE: Plot ${updatedPlot.id} → ${updatedPlot.statusText}');
    ////  });
    //  }
    //});
  }
  ...
  // UPDATED: _buildPlot with correct booked area color
