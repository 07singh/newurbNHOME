...
enum BookingStatus { available, pending, booked, approved, sellout }
...
class PlotInfo {
  ...
  factory PlotInfo.fromJson(Map<String, dynamic> json) {
    ...
    final parsedArea = double.tryParse(area.replaceAll(RegExp(r'[^0-9.]'), '')) ?? 0.0;
    final effectiveArea = areaUnit == AreaUnit.sqFt ? parsedArea / 9 : parsedArea;

    final totalAmount = (json['Total_Amount'] is num
        ? (json['Total_Amount'] as num).toDouble()
        : double.tryParse(json['Total_Amount']?.toString() ?? '0') ?? 0.0);
    final fare = effectiveArea > 0 ? totalAmount / effectiveArea : 0.0;
...
class _BookingDialogState extends State<BookingDialog> {
  ...
  Future<http.Response> _submitBookingAPI(PlotInfo plot) async {
    ...
    try {
      ...
      setState(() {
        _lastErrorMessage = 'Failed to check network connectivity: $e';
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    if (_uploadedPhoto != null) {
      try {
        ...
        setState(() {
          _lastErrorMessage = 'Failed to encode image: $e';
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
    ...
    print('Payload size: ${jsonBody.length} bytes');
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        print('Attempt $attempt - Response status: ${response.statusCode}, body: ${response.body}');
        ...
        print('Error submitting booking (attempt $attempt): $e');
        String errorMessage = e.toString();
        if (e is TimeoutException) {
          errorMessage = 'Request timed out after 30 seconds. Please check the server or network.';
        } else if (e is SocketException) {
          errorMessage = 'Network error: Unable to connect to the server. Please check your internet.';
        }
        setState(() {
          _lastErrorMessage = errorMessage;
        });
  ...
  void _updateTotalAmount() {
    ...
    final areaInYds = _areaUnit == AreaUnit.sqFt ? area / 9 : area;

    setState(() {
      _totalAmount = fare * areaInYds;
      final paid = double.tryParse(_paidController.text) ?? 0;
      _pendingController.text = (_totalAmount - paid).toStringAsFixed(0);
      print('Fare: $fare, Area: $areaInYds Sq.Yds, Total Amount: $_totalAmount');
    });
  }
                    ...
                    children: [
                      ...
                      _buildTextField(
                        ...
                        onChanged: (value) {
                          ...
                          setState(() {
                            _pendingController.text = (_totalAmount - paid).toStringAsFixed(0);
                            print('Paid Amount: $paid, Pending Amount: ${_totalAmount - paid}');
                          });
                        },
                        validator: (value) {
                          final paid = double.tryParse(value ?? '') ?? 0;
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (widget.plot.statusText.contains('Pending'))
                                    Text(
                                      '(${widget.plot.paidPercentage.toStringAsFixed(1)}%)',
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: widget.plot.color,
                                      ),
                        ...
                        child: ElevatedButton(
                          onPressed: (_isLoading || !_isSubmitEnabled || bookedArea > totalArea || _totalAmount <= 0)
                            ...
                            if (_formKey.currentState!.validate()) {
                              ...
                              if (widget.plot.bookingStatus != BookingStatus.approved && widget.plot.bookingStatus != BookingStatus.sellout) {
                                if (bookedArea > 0) {
                                  if (paidAmount >= _totalAmount &&
                                      bookedArea >= widget.plot._parseArea(widget.plot.area, widget.plot.areaUnit)) {
                              ...
                              try {
                                ...
                                if (response.statusCode == 200 || response.statusCode == 201) {
                                  try {
                                    final responseBody = jsonDecode(response.body);
                                    final apiStatus = responseBody['status']?.toString().toLowerCase();
                                    switch (apiStatus) {
                                      case 'approved':
                                        widget.plot.bookingStatus = BookingStatus.approved;
                                        break;
                                      case 'booked':
                                        widget.plot.bookingStatus = BookingStatus.booked;
                                        break;
                                      case 'sellout':
                                        widget.plot.bookingStatus = BookingStatus.sellout;
                                        break;
                                      case 'pending':
                                        ...
                                        break;
                                      default:
                                        print('Unknown API status: $apiStatus');
                                    }
                                  } catch (_) {
                                    // Keep current status if response parsing fails
                                  }
                                  widget.onSave(widget.plot);
                                  Navigator.pop(context);
                                } else {
                                  // Revert to original plot on failure
                                  widget.plot.clientName = originalPlot.clientName;
                                  widget.plot.fare = originalPlot.fare;
                                  widget.plot.paidAmount = originalPlot.paidAmount;
                                  widget.plot.totalAmount = originalPlot.totalAmount;
                                  widget.plot.bookedArea = originalPlot.bookedArea;
                                  widget.plot.area = originalPlot.area;
                                  widget.plot.areaUnit = originalPlot.areaUnit;
                                  widget.plot.photoPath = originalPlot.photoPath;
                                  widget.plot.projectName = originalPlot.projectName;
                                  widget.plot.purchasePrice = originalPlot.purchasePrice;
                                  widget.plot.receivingAmount = originalPlot.receivingAmount;
                                  widget.plot.pendingAmount = originalPlot.pendingAmount;
                                  widget.plot.paidThrough = originalPlot.paidThrough;
                                  widget.plot.bookedByDealer = originalPlot.bookedByDealer;
                                  widget.plot.customerPhone = originalPlot.customerPhone;
                                  widget.plot.dealerPhone = originalPlot.dealerPhone;
                                  widget.plot.status = originalPlot.status;
                                  widget.plot.bookingStatus = originalPlot.bookingStatus;
                                  widget.plot.plotType = originalPlot.plotType;
                                }
                              } catch (e) {
                                // Revert to original plot on error
                                widget.plot.clientName = originalPlot.clientName;
                                widget.plot.fare = originalPlot.fare;
                                widget.plot.paidAmount = originalPlot.paidAmount;
                                widget.plot.totalAmount = originalPlot.totalAmount;
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStatusItem(Colors.blue.shade500, 'Approved'),
