...
class _WeekFlowupPageState extends State<WeekFlowupPage> {
  ...
  Future<void> _updateFollowUp(Map<String, dynamic> item) async {
    final nextDateString = item["Next_FollowUp_Date"]?.toString();
    DateTime? selectedDate =
        nextDateString != null && nextDateString.isNotEmpty ? DateTime.tryParse(nextDateString) : DateTime.now();
    final remarkController = TextEditingController(text: item["Last_Remark"] ?? "");
  ...
  Future<void> _fetchFollowUps() async {
    if (_userPhone == null || _userPhone!.isEmpty) {
      setState(() {
        _followUps.clear();
        _error = "User phone not available";
      });
      return;
    }
  ...
  Widget _buildList() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final name = item["Client_Name"] ?? "-";
        final contact = item["Contact_No"] ?? "-";
        final project = item["Project_Name"] ?? "-";
        final last = _formatDate(item["Last_FollowUp_Date"]?.toString());
        final next = _formatDate(item["Next_FollowUp_Date"]?.toString());
        final remark = item["Last_Remark"] ?? "No note";
        final phone = item["Contact_No"]?.toString() ?? "";
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Chip(
                    label: Text("ID #${item["FollowUp_Id"] ?? "-"}"),
  ...
  bool _matchesFilter(Map<String, dynamic> item) {
    final nextDateString = item["Next_FollowUp_Date"]?.toString();
    if (nextDateString == null || nextDateString.isEmpty) return false;
